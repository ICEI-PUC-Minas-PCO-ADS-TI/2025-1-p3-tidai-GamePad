...
export default function ListCreate() {
  ...
  const handleSave = async () => {
    ...
    const payload = {
      title,
      userId: user.id,
      items: games.map((g) => g.id), // tente trocar para: games.map(g => ({ gameId: g.id })) se necessário
    };
    ...
    if (res.ok) {
      ...
      navigate(`/${user.nome.toLowerCase().replace(/\s+/g, "-")}/list/${data.id}`);
    } else {
  ...
  return (
    <div className="max-w-2xl mx-auto bg-zinc-900 p-8  mt-8 ">
      <h2 className="text-2xl font-bold text-cyan-400 mb-6">Criar nova lista</h2>
      <div className="mb-4">
        <label className="block text-zinc-200 font-semibold mb-1">Nome da lista</label>
        <input
      ...
      <div className="mb-4">
        ...
        <div className="flex flex-wrap gap-2">
          {games.map((game) => {
            ...
            if (game.cover && game.cover.url) {
              cover = game.cover.url.startsWith('http')
                ? game.cover.url.replace(/t_thumb|t_cover_small/g, 't_cover_big')
                : `https:${game.cover.url.replace(/t_thumb|t_cover_small/g, 't_cover_big')}`;
            ...
            return (
              <div key={game.id} className="flex flex-col items-center bg-zinc-800 rounded-lg p-0 w-24 h-32 relative overflow-hidden">
                <img src={cover} alt={game.name} className="w-full h-full object-cover rounded mb-0" />
                <span className="text-xs text-center line-clamp-2 absolute bottom-1 left-0 w-full bg-black/60 text-white px-1 py-0.5">{game.name}</span>
                <button
          ...
          {games.length === 0 && <span className="text-zinc-500 text-xs">Nenhum jogo adicionado.</span>}
        </div>
      </div>
